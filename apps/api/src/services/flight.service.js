...
class FlightService {
    constructor() {
        this.cachePrefix = 'flight:';
        this.searchCacheTTL = 300; // 5 minutes
        this.priceHistoryCacheTTL = 3600; // 1 hour
    }
    search(input, userId) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                const cachedResult = yield CacheService.get(cacheKey);
                if (cachedResult) {
                ...
                if (userId) {
                    yield this.storeSearchQuery(input, userId);
                }
                // Search flights from multiple providers
                const results = yield this.searchFromProviders(input);
                // Cache the results
                yield CacheService.set(cacheKey, results, this.searchCacheTTL);
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const result = {
                ...
                summary: {
                    ...
                    averagePrice: priceHistory.reduce((sum, p) => sum + p.avgPrice, 0) /
                        priceHistory.length,
                    highestPrice: Math.max(...priceHistory.map(p => p.maxPrice)),
                },
            };
            // Cache for 1 hour
            yield CacheService.set(cacheKey, result, this.priceHistoryCacheTTL);
            return result;
        });
    }
    createPriceAlert(input) {
        return __awaiter(this, void 0, void 0, function* () {
            const alert = yield prisma.priceAlert.create({
                data: {
                    userId: input.userId,
                    origin: input.origin,
                    destination: input.destination,
    ...
    getUserPriceAlerts(userId) {
        return __awaiter(this, void 0, void 0, function* () {
            const alerts = yield prisma.priceAlert.findMany({
                where: {
                    userId,
                    isActive: true,
                },
    ...
    deletePriceAlert(alertId, userId) {
        return __awaiter(this, void 0, void 0, function* () {
            const alert = yield prisma.priceAlert.findFirst({
                where: {
                    id: alertId,
                    userId,
                },
            ...
            yield prisma.priceAlert.update({
                where: { id: alertId },
                data: { isActive: false },
            });
    ...
    storeSearchQuery(input, userId) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                yield prisma.flightSearch.create({
                    data: {
                        userId,
                        origin: input.origin,
                        destination: input.destination,
    ...
    generateSearchCacheKey(input) {
        ...
        if (input.maxPrice)
            key += `:maxPrice${input.maxPrice}`;
        if ((_a = input.airlines) === null || _a === void 0 ? void 0 : _a.length)
            key += `:airlines${input.airlines.sort().join(',')}`;
        if (input.stops !== undefined)
            key += `:stops${input.stops}`;
        return key;
    }
    searchFromProviders(input) {
        return __awaiter(this, void 0, void 0, function* () {
