...
export const analyticsRouter = router({
  ...
  trending: baseProcedure
    ...
    .query(async ({ input }) => {
      if (input.type === 'destinations') {
        return await prisma.trendingDestination.findMany({
          where: {
            period: input.period,
            periodStart: {
        ...
        return await prisma.popularRoute.findMany({
          where: {
            period: input.period,
            periodStart: {
              gte: new Date(new Date().setMonth(new Date().getMonth() - 1)),
      ...
      const priceHistory = await prisma.priceHistory.findMany({
        where: {
          ...
          date: {
            ...
            lt: new Date(
              new Date(input.departureMonth + '-01').setMonth(
                new Date(input.departureMonth + '-01').getMonth() + 1
              )
            ),
          },
        },
  ...
  systemMetrics: adminProcedure.query(async () => {
    const [totalUsers, totalSearches, totalBookings, activeAlerts] =
      await Promise.all([
        prisma.user.count(),
        prisma.flightSearch.count(),
        prisma.booking.count(),
    ...
    return {
      ...
      bookings: {
        ...
        conversion_rate:
          totalSearches > 0 ? (totalBookings / totalSearches) * 100 : 0,
      },
      alerts: {
        active: activeAlerts,
